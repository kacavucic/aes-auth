                ...
                <div class="${properties.kcFormGroupClass!}">
                    ...
                    <#if groupDisplayDescription != "">
                        ...
                        <div class="${properties.kcLabelWrapperClass!}">
                            <label id="description-${groupName}"
        ...
        <div class="${properties.kcFormGroupClass!}">
            <div class="${properties.kcLabelWrapperClass!}">
                <label for="${attribute.name}"
                       class="${properties.kcLabelClass!}">${advancedMsg(attribute.displayName!'')}</label>
            ...
            <div class="${properties.kcInputWrapperClass!}">
                ...
                <#if attribute.annotations.inputHelperTextAfter??>
                    <div class="${properties.kcInputHelperTextAfterClass!}" id="form-help-text-after-${attribute.name}"
...
<#macro inputFieldByType attribute>
    <#switch attribute.annotations.inputType!''>
        ...
        <#case 'multiselect-checkboxes'>
            ...
            <#break>
        <#case 'html5-tel'>
            <@inputTagTel attribute=attribute/>
            <#break>
        <#default>
...
<#macro inputTag attribute>
    <input type="<@inputTagType attribute=attribute/>" id="${attribute.name}" name="${attribute.name}"
           value="${(attribute.value!'')}" class="${properties.kcInputClass!}"
...
<#macro inputTagTel attribute>
    <input id="${attribute.name}" name="${attribute.name}" class="${properties.kcInputClass!}"
           aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
           <#if attribute.readOnly>disabled</#if> onfocusout="process(event)"/>
    <script>

        function getIp(callback) {
            fetch('https://ipinfo.io/json?token=132420b5956d40', {headers: {'Accept': 'application/json'}})
                .then((resp) => resp.json())
                .catch(() => {
                    return {
                        country: 'fr',
        ...
        const phoneInputField = document.querySelector("#${attribute.name}");
